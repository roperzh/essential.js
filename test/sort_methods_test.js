describe("Essential.Core.SortMethods", function() {
  describe("#byPriority", function() {
    ...
    it("orders an array of behaviors in descendent order based on their priorities", function() {
      FirstPriorityBehavior = Essential.Behavior.extend({ priority: 3 });
      SecondPriorityBehavior = Essential.Behavior.extend({ priority: 2 });
      ThirdPriorityBehavior = Essential.Behavior.extend({ priority: 1 });

      var simpleArray =
        [SecondPriorityBehavior,FirstPriorityBehavior, ThirdPriorityBehavior];
